...
function export_statement(exporter::PythonExporter, wb::ExcelWorkbook, statement::FunctionStatement)
    cell_ref = statement.assigned_var
    lhs = exporter.var_names[cell_ref]
...
function get_function_string(exporter::PythonExporter, wb::ExcelWorkbook, statement::FunctionStatement)
    needed_vars = copy(get_cell_deps(statement))
    for child in statement.intermediates
        append!(needed_vars, get_cell_deps(child))
    end
