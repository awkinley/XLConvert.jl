...
function get_statements(wb::XLConvert.ExcelWorkbook2)
    ...
    target_output = CellDependency("Summary Results", "D7")
    all_target_outputs = [target_output]
...
function run(wb_in::XLConvert.ExcelWorkbook2)
    ...
    target_output = CellDependency("Summary Results", "D14")
    all_target_outputs = [target_output]
    ...
    println("Write file")
    @time write_file(exporter, "nrel_crest_solar.jl", wb, statements)
